...
const getCross = (wire1, wire2) => {
  ...
  tracedPath1.forEach(([wire1Dir, wire1Point1, wire1Point2, wire1Steps]) => {

    found = tracedPath2.forEach( ([wire2Dir, wire2Point1, wire2Point2, wire2Steps]) => {
      if (wire1Dir == wire2Dir) return;

      if (wire1Dir == 'H') {
        areCrossed([wire1Point1, wire1Point2], [wire2Point1, wire2Point2]) && cross.push(wire1Steps - Math.abs(wire2Point2[1] - wire1Point1[1]) + wire2Steps - Math.abs(wire1Point2[0] - wire2Point1[0]));
      } else {
        areCrossed([wire2Point1, wire2Point2], [wire1Point1, wire1Point2]) && cross.push(wire1Steps - Math.abs(wire2Point2[0] - wire1Point1[0]) + wire2Steps - Math.abs(wire1Point2[1] - wire2Point1[1]));
      }
    });
  });
...
readFile('./input', "utf8", (err, data) => {
  ...
  const res = Math.min(...getCross(paths[0], paths[1]));

  console.log(res);
  assert.equal(res, 20286);
});